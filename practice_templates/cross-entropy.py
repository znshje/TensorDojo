...
def solve(logits, targets, ignore_index=-100):
    """交叉熵与 ignore_index：在此实现。"""
    # TODO: 根据题目约定返回结果
...
if __name__ == "__main__":
    ...
    logits = torch.tensor([[1.1334044055743933, 0.4620772467152532, 0.04206122944820574, 0.34594601821099746,
      -1.5945281557811837],
     [-1.3358975990189317, 0.39975937921625415, -0.5182359019396884, 0.00892651332797321,
      -1.0717673967531753],
     [0.18870746079472087, -0.5132712934449802, -0.41126789034091465, -2.290042350672562,
      0.24982736396850957],
     [-0.5754154776340655, 0.3394419912229643, -1.1676854170730067, 0.28562991374400915,
      -0.15793455122745947]], dtype=torch.float64).reshape((4, 5))
    targets = torch.tensor([0, 1, 3, 4], dtype=torch.int64).reshape((4,))
    expected = torch.tensor(1.7658450211738388, dtype=torch.float64).reshape(())
    try:
